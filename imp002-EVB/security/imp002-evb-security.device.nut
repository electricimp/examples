...
// force the imp to connect
server.log("Awake");
// show us the imp is awake
...
// check to see if we woke on an interrupt and alert the agent if we did
...
// everything is set up, now go to sleep
imp.onidle(function() {
    server.sleepfor(86400); // sleep for one day
});
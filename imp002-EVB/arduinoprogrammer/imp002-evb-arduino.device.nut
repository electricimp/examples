...
/****************[ instructions ]****************
 * 
 * You will need to install the "optiboot" bootloader on the Arduino using an ICSP cable.
 * See https://github.com/electricimp/reference/tree/master/hardware/impeeduino for instructions.
 * 
 * Wire from EVB 5V0 to Arduino 5V
 * Wire from EVB GND to Arduino GND 
 * Wire from EVB PIN6 (TX) to EVB <- 3.3V 
 * Wire from EVB PINE (RX) to EVB 3.3V -> 
 * Wire from EVB PINB to EVB FET2 
 * Wire from EVB OD2 to Arduino RESET 
 * Wire from EVB <- 5.0V to Arduino PIN1 (TX) 
 * Wire from EVB 5.0V -> to Arduino PIN0 (RX) 
 * 
 * If there isn't one already there, place an LED on between Arduino PIN13 and Arduino GND
 * 
 * Load the Arduino IDE
 * Open the Preferences window and tick "Show verbose output during [x] compilation [_] upload"
 * Create a new project using the example app "Blink"
 * Click the Verify button to compile the project
 * The second-last line of the logs will be the HEX file location, such as /var/folders/v5/76186smn23z0m5p5894ht6qw0000gn/T/build1034562346335785183.tmp/Blink.cpp.hex
 * Open the agent URL in your browser
 * Click Choose File and select the HEX file above
 * Click Press to upload the file.
 * Wait for the Arduino to be programmed.
 * Watch the LED on the Arduino blink every second.
 * 
 */
...
function bounce() {
    ...
    imp.sleep(0.5);
    ACTIVITY.write(1-ACTIVITY.read());
    RESET.write(1);
    imp.sleep(0.2);
    RESET.write(0);
    imp.sleep(0.3);

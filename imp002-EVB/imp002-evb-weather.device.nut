...
const NUMPIXELS = 12; // set the number of pixels you're using here
...
class NeoWeather extends NeoPixels {
    ...
    constructor(_frameSize) {
        hardware.spi257.configure(MSB_FIRST, 7500);
        base.constructor(hardware.spi257, _frameSize);
        pixelvalues = [];
...
//------------------------------------------------------------------------------
agent.on("weather", function(weather) {
    if (!("conditions" in weather)) return;
    server.log(format("Conditions: %s, Temperature: %0.01f °C", weather.conditions, weather.temperature))
...
//------------------------------------------------------------------------------
display <- NeoWeather(NUMPIXELS);
agent.send("weather", true);
    ...
    function buttonReadHandler(err, res) {
        if (err) {
            ...
            if (state != null && state != pump_btn) {
                ...
                (state) ? FieldbusGateway_005.LED_RED.write(FB_LED_ON) : FieldbusGateway_005.LED_RED.write(FB_LED_OFF);
                ...
                if (tank_level < DEFAULT_LEVEL_MAX) {
                    ...
                    (state) ? FieldbusGateway_005.LED_GREEN.write(FB_LED_ON) : FieldbusGateway_005.LED_GREEN.write(FB_LED_OFF);
                }
            }
            imp.wakeup(0.05, function() { FieldbusGateway_005.LED_YELLOW.write(FB_LED_ON) }.bindenv(this));
        }
    }
